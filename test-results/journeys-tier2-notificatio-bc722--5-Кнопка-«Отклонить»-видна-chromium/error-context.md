...
- Name: journeys-tier2-notifications.spec.ts >> Journey 35: Подтверждение и отклонение бронирования >> 35.5 Кнопка «Отклонить» видна
- Location: e2e\journeys-tier2-notifications.spec.ts:231:7
...
Locator: locator('div.border-b').filter({ hasText: 'Клиент Отказ' }).first().getByRole('button', { name: /Отклонить/i })
Expected: visible
Timeout: 5000ms
