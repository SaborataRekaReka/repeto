...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 31: DebtList — задолженности и навигация >> 31.3 Сумма долга отображается красным
- Location: e2e\journeys-tier2-dashboard.spec.ts:230:7
...
Locator: locator('.card').filter({ hasText: 'Задолженности' }).locator('.text-pink-1').first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Задолженности' }).locator('.text-pink-1').first()
