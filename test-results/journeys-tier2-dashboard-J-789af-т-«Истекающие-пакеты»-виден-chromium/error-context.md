...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 33: Занятия на неделю и истекающие пакеты >> 33.4 Виджет «Истекающие пакеты» виден
- Location: e2e\journeys-tier2-dashboard.spec.ts:394:7
...
Locator: locator('.card').filter({ hasText: 'Истекающие пакеты' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Истекающие пакеты' })
