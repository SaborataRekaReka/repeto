...
- Name: journeys-tier2-finance.spec.ts >> Journey 46: Пакеты — CRUD и фильтры >> 46.1 Заголовок «Пакеты занятий» виден
- Location: e2e\journeys-tier2-finance.spec.ts:281:7
...
Locator: locator('.text-h3, .text-h4').filter({ hasText: 'Пакеты занятий' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.text-h3, .text-h4').filter({ hasText: 'Пакеты занятий' }).first()
