...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 30: TodaySchedule — виджет и LessonDetailModal >> 30.1 Виджет «Сегодня» — заголовок с датой
- Location: e2e\journeys-tier2-dashboard.spec.ts:135:7
...
Locator: locator('.card').filter({ hasText: 'Сегодня,' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Сегодня,' })
