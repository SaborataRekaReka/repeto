...
- Name: journeys-tier2-finance.spec.ts >> Journey 44: Финансовый обзор — виджеты и навигация >> 44.6 IncomeChart — переключатель периода (dropdown) видим
- Location: e2e\journeys-tier2-finance.spec.ts:110:7
...
Locator: locator('.card').filter({ hasText: 'Доход' }).filter({ hasText: 'Получено' }).getByText('Месяц')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Доход' }).filter({ hasText: 'Получено' }).getByText('Месяц')
