...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.7 Итого за период — содержит ₽
- Location: e2e\journeys-tier2-dashboard.spec.ts:326:7
...
Locator: locator('.card').filter({ hasText: 'Итого за период' }).locator('.text-h5')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Итого за период' }).locator('.text-h5')
