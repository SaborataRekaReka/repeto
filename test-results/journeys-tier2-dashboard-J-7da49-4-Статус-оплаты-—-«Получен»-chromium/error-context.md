...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.4 Статус оплаты — «Получен»
- Location: e2e\journeys-tier2-dashboard.spec.ts:311:7
...
Locator: locator('.card').filter({ hasText: 'Последние оплаты' }).getByText('Получен')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Последние оплаты' }).getByText('Получен')
