...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.3 Сумма оплаты видна
- Location: e2e\journeys-tier2-dashboard.spec.ts:306:7
...
Locator: locator('.card').filter({ hasText: 'Последние оплаты' }).getByText(/3[\s\u00a0]?000/)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Последние оплаты' }).getByText(/3[\s\u00a0]?000/)
