...
- Name: journeys-tier2-student-card.spec.ts >> Journey 23: URL-синхронизация табов >> 23.1 Без ?tab= → таб «Занятия» активен
- Location: e2e\journeys-tier2-student-card.spec.ts:541:7
...
Locator: getByRole('button', { name: /Назначить занятие/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for getByRole('button', { name: /Назначить занятие/i })
...
- generic [active] [ref=e1]:
  - generic [ref=e2]:
    - complementary [ref=e3]:
      ...
      - generic [ref=e50]:
        ...
        - button "ТU Тест URL Табы" [ref=e69] [cursor=pointer]:
          - generic [ref=e70]: ТU
          - generic [ref=e71]: Тест URL Табы
    ...
    - main [ref=e80]:
      - generic [ref=e81]:
        - generic [ref=e82]:
          - button [ref=e83] [cursor=pointer]:
            - img [ref=e86]:
              - img [ref=e87]
          - generic [ref=e89]: Табовый Ученик
        - generic [ref=e90]:
      ...
      - generic [ref=e112]:
        - complementary [ref=e113]:
          - generic [ref=e114]:
            - generic [ref=e115]: ТУ
            - generic [ref=e116]: Табовый Ученик
            - generic [ref=e117]: История
            - generic [ref=e118]:
  ...
  - alert [ref=e175]: /students/82eef200-79e2-41a7-8565-635f57964680
```
...
> 548 |     await expect(page.getByRole('button', { name: /Назначить занятие/i })).toBeVisible();
  ...
  574 |   });
  575 | 
  576 |   test('23.5 Прямой переход ?tab=notes → таб «Заметки» активен', async () => {
  577 |     await page.goto(`/students/${studentId}?tab=notes`);
  578 |     await page.waitForTimeout(1_000);
  579 | 
  580 |     await expect(page.getByRole('button', { name: /Добавить/i })).toBeVisible({ timeout: 5_000 });
  581 |   });
  582 | 
  583 |   test('23.6 Клик по «Занятия» → ?tab= убирается из URL', async () => {
  584 |     await page.getByText('Занятия').first().click();
  585 |     await page.waitForTimeout(500);
  586 | 
  587 |     // При возврате на Занятия — ?tab= не добавляется (дефолтный таб)
  588 |     expect(page.url()).not.toContain('tab=');
  589 |     await expect(page.getByRole('button', { name: /Назначить занятие/i })).toBeVisible();
  590 |   });
  591 | });
  592 | 
```