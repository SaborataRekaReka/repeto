...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 33: Занятия на неделю и истекающие пакеты >> 33.3 Клик по уроку → LessonDetailModal
- Location: e2e\journeys-tier2-dashboard.spec.ts:382:7
...
Call log:
  - waiting for locator('.card').filter({ hasText: 'Занятия на неделю' }).locator('button').first()
