...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 33: Занятия на неделю и истекающие пакеты >> 33.1 Виджет «Занятия на неделю» виден
- Location: e2e\journeys-tier2-dashboard.spec.ts:372:7
...
Locator: locator('.card').filter({ hasText: 'Занятия на неделю' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Занятия на неделю' })
