...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 33: Занятия на неделю и истекающие пакеты >> 33.2 Урок на неделю отображается
- Location: e2e\journeys-tier2-dashboard.spec.ts:377:7
...
Locator: locator('.card').filter({ hasText: 'Занятия на неделю' }).getByText(/Математика/)
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Занятия на неделю' }).getByText(/Математика/)
