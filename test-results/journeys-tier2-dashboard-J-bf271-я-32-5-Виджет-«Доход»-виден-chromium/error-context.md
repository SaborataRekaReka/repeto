...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.5 Виджет «Доход» виден
- Location: e2e\journeys-tier2-dashboard.spec.ts:316:7
...
Locator: locator('.card').filter({ hasText: 'Итого за период' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Итого за период' })
