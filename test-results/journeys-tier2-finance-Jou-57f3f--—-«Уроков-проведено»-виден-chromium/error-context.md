...
- Name: journeys-tier2-finance.spec.ts >> Journey 44: Финансовый обзор — виджеты и навигация >> 44.7 PeriodSummary — «Уроков проведено» виден
- Location: e2e\journeys-tier2-finance.spec.ts:116:7
...
Locator: locator('.card').filter({ hasText: 'Сводка за период' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Сводка за период' })
