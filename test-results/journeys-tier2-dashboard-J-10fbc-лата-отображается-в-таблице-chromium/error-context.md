...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.2 Оплата отображается в таблице
- Location: e2e\journeys-tier2-dashboard.spec.ts:301:7
...
Locator: locator('.card').filter({ hasText: 'Последние оплаты' }).getByText('Платящий Тест')
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Последние оплаты' }).getByText('Платящий Тест')
