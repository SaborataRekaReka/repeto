...
- Name: buttons-interactivity.spec.ts >> Кнопки — все интерактивные элементы работают >> Payments: кнопка "Записать оплату" открывает модал
- Location: e2e\buttons-interactivity.spec.ts:37:7
...
Locator: getByRole('button', { name: /Записать оплату/i }).first()
Expected: visible
Timeout: 10000ms
