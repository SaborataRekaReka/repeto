...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.6 Получено — сумма отображается
- Location: e2e\journeys-tier2-dashboard.spec.ts:321:7
...
Locator: locator('.card').filter({ hasText: 'Итого за период' }).getByText(/Получено/).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Итого за период' }).getByText(/Получено/).first()
