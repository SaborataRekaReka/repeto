...
- Name: journeys-tier2-notifications.spec.ts >> Journey 35: Подтверждение и отклонение бронирования >> 35.2 Кнопка «Подтвердить» видна
- Location: e2e\journeys-tier2-notifications.spec.ts:211:7
...
Locator: locator('div.border-b').filter({ hasText: 'Клиент Подтверд' }).first().getByRole('button', { name: /Подтвердить/i })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('div.border-b').filter({ hasText: 'Клиент Подтверд' }).first().getByRole('button', { name: /Подтвердить/i })
