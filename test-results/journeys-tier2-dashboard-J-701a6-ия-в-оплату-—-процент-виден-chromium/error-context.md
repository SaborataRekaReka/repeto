...
- Name: journeys-tier2-dashboard.spec.ts >> Journey 32: Последние оплаты, доход и конверсия >> 32.8 Конверсия в оплату — процент виден
- Location: e2e\journeys-tier2-dashboard.spec.ts:335:7
...
Locator: locator('.card').filter({ hasText: 'Конверсия в оплату' })
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.card').filter({ hasText: 'Конверсия в оплату' })
