...
- Name: journeys-tier2-finance.spec.ts >> Journey 45: Оплаты — таблица, фильтры, детали >> 45.1 Заголовок «Оплаты» виден
- Location: e2e\journeys-tier2-finance.spec.ts:187:7
...
Locator: locator('.text-h3, .text-h4').filter({ hasText: 'Оплаты' }).first()
Expected: visible
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('.text-h3, .text-h4').filter({ hasText: 'Оплаты' }).first()
